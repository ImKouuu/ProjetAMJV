...
public class Projectile : MonoBehaviour
{
    ...
    private bool HitEnnemy(Collider other)
    {
        ...
        {
            UnitController hitCharacter = other.GetComponent<UnitController>();

            if (hitCharacter.GetComponent<UnitController>().tag != data.launcherTag)
            {
                hitCharacter.TakeDamage((int)data.damage);
                return true;
            }
        }
    ...
    {
        if(other.tag == "UnitController")
        {
            return other.GetComponent<UnitController>().tag == data.launcherTag;
        }
